...
AGB_MAP <- raster('./Maps/Mexico_biomass_100m_2020.tif')
proj4string(AGB_MAP) <- CRS("+proj=longlat +datum=WGS84 +units=m")
AGB_MAP[AGB_MAP<0] = 0
plot(AGB_MAP)
...
AGB_MAP_extract <- extract(AGB_MAP,sp_dat, buffer=radius, fun=mean)
sp_dat$AGB_MAPe <- AGB_MAP_extract
...
sp_dat$AGB_MAP <- sp_dat$AGB_MAPe/10
...
cover_t = st_read('./Mexico/Land_use/SerieVII_wgs84/Usv_SVII_MGM2021_wgs84.shp')%>% st_make_valid()
head(cover_t)
#cover = st_as_sf(cover_t,CRS("+proj=longlat +datum=WGS84 +units=m")) #%>% st_make_valid()

head(cover_t)
#AGB_15_ex <- extract(AGB_MAP,states, fun=mean)
#writeRaster(AGB_MAP, 'Mexico_AGB_MAP_100m_00.tif')

sp_dat_states = st_join(sp_dat, states, left=T)
sp_dat_states_cov = st_join(sp_dat_states, cover_t, left=T)
head(sp_dat_states_cov)


# write.csv(sp_dat_states, 'Mex_NFI_Ctrees_AGB_2020.csv')

# Data Binning
# create bins of 5 mgha
bins = seq(0,max(na.omit(sp_dat_states_cov$AGB_MAP)), by=5)
length(bins)
# AGB_bins = sp_dat_states_cov %>% mutate(new_bin = cut(sp_dat_states_cov$`Daso_A_Biomasa_aerea_Ton/HA`, bins))
AGB_bins = sp_dat_states_cov %>% mutate(points_bin = ntile(`Daso_A_Biomasa_aerea_Ton/HA`, n=87))
# AGB_bins = sp_dat_states_cov %>% mutate(points_bin = ntile(AGB_MAP, n=87))
head(AGB_bins)
# write.csv(AGB_bins, 'Mex_NFI_Ctrees_AGB_2020_bins.csv')
# AGB_by_bins = AGB_bins %>% group_by(points_bin) %>%
#   summarise(mean_agb_map=mean(AGB_MAP))

AGB_by_bins = AGB_bins %>% group_by(points_bin) %>%
  summarise(mean_agb_map=median(AGB_MAP, na.rm =T),
          mean_agb=median(AGB, na.rm =T))

# write.csv(AGB_by_bins, 'Mex_NFI_Ctrees_AGB_2020_AGB_by_bins.csv')
head(AGB_by_bins)

AGB_by_bins_cov = st_join(AGB_by_bins, cover_t, left = T)

mod = lm(mean_agb_map~mean_agb, data = AGB_by_bins)

plot(AGB_by_bins$mean_agb_map, AGB_by_bins$mean_agb)
abline(mod)
ggplot(data = AGB_by_bins, mapping = aes(x = mean_agb, y = mean_agb_map)) +
  geom_pointdensity() +
...
# newdata = na.omit(sp_dat_states_cov)
# newdata <- subset(sp_dat_states, AGB_MAP >= 0)
# newdata <- subset(newdata, AGB_2020 >= 0)
# newdata <- na.omit(newdata)
# 
# 
# plot(newdata$AGB_MAP)


## make some plots

# lm(AGB~AGB_MAP, data = newdata)
# 
# ggplot(data = newdata, mapping = aes(x = AGB_MAP, y = AGB)) +
#   geom_pointdensity() +
#   scale_color_viridis()+
#   stat_poly_line() +
#   geom_abline(intercept = 0, slope = 1, lty=2, lwd=2)+
...
# plot  mapped AGB vs Field estimates by cover type (binned)
#mean_agb~mean_agb_map
ggp <- ggplot(AGB_by_bins_cov, aes(y = mean_agb_map, x = mean_agb)) + 
  xlim(0,200) + ylim(0,200) +
  geom_pointdensity() +
  scale_color_viridis()+
...
# plot mapped AGB vs Field estimates by cover type
# head(newdata)
# ggp <- ggplot(newdata, aes(x = AGB_MAP, y = AGB)) +           
#   geom_pointdensity() +
#   scale_color_viridis()+
#   facet_wrap( ~ StratoForm)
# #ggp
...
#2015
ggplot(data = newdata, aes(x = AGB_MAP, y = AGB, colour = Formacion_S6_C4)) +
...
ggplot(newdata,aes(x=AGB_MAP,y=NOM_ENT, na.rm=T))+geom_boxplot() #+
...
ggplot(newdata,aes(x=AGB_MAP,y=Formacion_S6_C4, na.rm=T))+geom_boxplot() #+
...
AGB_by_state_tbl <- newdata %>% group_by(NOM_ENT) %>%
  summarise(mean_agb.y=mean(AGB),
            sd_agb=sd(AGB),
            var_agb=var(AGB),
            Mean_2015 = mean(AGB_MAP),
            sd_2015 = sd(AGB_MAP),
            var_2015 = var(AGB_MAP),
...
boxplot(newdata$AGB, newdata$AGB_MAP, newdata$AGB_2020, newdata$GEDI_25m )


# df <- cbind(newdata$IdConglomerado,newdata$AGB,  newdata$AGB_MAP, newdata$AGB_2020, newdata$NOM_ENT, newdata$Formacion_S6_C4)
# colnames(df) <- c('IdConglomerado', 'AGB', 'AGB_MAP', 'AGB_2020', 'NOM_ENT', 'FORMACION')
# df <- data.frame(df)
# 
# agb1 <- cbind(df$AGB, 'AGB', df$NOM_ENT, df$FORMACION)
# agb2 <- cbind(df$AGB_MAP, 'AGB_MAP', df$NOM_ENT, df$FORMACION)
